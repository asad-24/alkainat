...
interface EditTeacherModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (teacherId: string, teacher: Omit<Teacher, '_id' | 'createdAt' | 'updatedAt'>) => void;
    ...
    useEffect(() => {
        if (teacher && isOpen) {
            setFormData({
                ...
                degrees: teacher.degrees && teacher.degrees.length > 0 ? teacher.degrees : [{ degree: '', university: '' }],
                gender: teacher.gender || 'male',
                backgrounds: teacher.backgrounds && teacher.backgrounds.length > 0 ? teacher.backgrounds : [''],
                languages: teacher.languages && teacher.languages.length > 0 ? teacher.languages : [''],
            });
        }
    }, [teacher, isOpen]);
            ...
            <DialogContent className="sm:max-w-[600px] h-[80vh] bg-slate-900/95 backdrop-blur-xl border-white/10 overflow-y-auto">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" disabled={loading} className="bg-gradient-to-r from-purple-500 to-cyan-500 hover:from-purple-600 hover:to-cyan-600 text-white border-0">{loading ? 'Updating...' : 'Update Teacher'}</Button>
                    </DialogFooter>
                </form>
            </DialogContent>
        </Dialog>
